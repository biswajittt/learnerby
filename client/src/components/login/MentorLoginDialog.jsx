...
import { List, ListItem, Box, Button, Dialog, TextField, Autocomplete, Typography, styled, FormControl, InputLabel, Select, MenuItem, Chip, OutlinedInput, useTheme, Input, Avatar, MobileStepper, Stack } from "@mui/material";
...
export default function MentorLoginDialog(props) {
    ...
    const [error, setError] = useState({ errorAvailable: false, errorText: '' });
    ...
    const handleClose = () => {
        ...
        setRegistrationData(initialRegistrationValue);
        setError(false);
    ...
    const loginMentor = async () => {
        ...
        console.log(error.errorAvailable)
        console.log(loginData)

        if (loginData.email === '' || loginData.password === '') {
            console.log("empty")
            setError({ errorAvailable: true, errorText: "Please enter your login data" });
            console.log(error.errorAvailable)
            setLoginButton({ text: 'Login', isClicked: false, createAccountText: 'block' })
            return;
        }
        if (!isValidateEmail(loginData.email.toLowerCase())) {
            ...
            setError({ errorAvailable: true, errorText: "Please enter a valid email Id" });
            console.log(error.errorAvailable)
            setLoginButton({ text: 'Login', isClicked: false, createAccountText: 'block' })
            return;
        }
        ...
        if (response.status === 200) {
            ...
            setLoginButton({ text: 'Login', isClicked: false, createAccountText: 'block' })
            setError({ errorAvailable: false, errorText: "Please enter valid login data" });
        }
            ...
            console.log('error')
            setLoginButton({ text: 'Login', isClicked: false, createAccountText: 'block' })
            setError({ errorAvailable: false, errorText: "Please provide valid login data" });
            // setError(true);
            // setLoginButton({ text: 'Login', isClicked: false, createAccountText: 'block' })
        }
    }
                    ...
                    >
                        {
                            account.view === "login" ?
                                ...
                                <RightContainer>
                                    ...
                                    <CreateAccountText onClick={toggleRegistration} sx={{ display: loginButton.createAccountText }}>
                                        New to Learnerby? Create an mentor account
                                    </CreateAccountText>
                                ...
                                <RightContainer>
                                    ...
                                    <Box style={{ display: 'flex', justifyContent: 'center' }}>
                                        <label htmlFor='mentor-profile-avater' style={{ cursor: "pointer" }}>
                                            <Avatar
                                    ...
                                    </Box>
                                    {
                                        (activeStep == 0) ?
                                            ...
                                            <Stack spacing={2} sx={{ height: "315px" }}>
                                            ...
                                            <Stack spacing={2} sx={{ height: "360px" }}>
